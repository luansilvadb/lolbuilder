...
func renderManifest(in Input, arquivos []File) string {
	...
	fmt.Fprintf(&b, "| Grupos de exclusividade de item | %d |\n\n", len(ds.GruposDeItem))
	...
	b.WriteString("| `cdragon`, dump do jogo | estatística base, fórmula de habilidade e grupos de exclusividade |\n")
	...
	b.WriteString("- **Itens do mesmo grupo de exclusividade não se acumulam.** Comprar um trava\n")
	b.WriteString("  os outros na loja. Antes de propor qualquer build, confira a coluna `grupos`\n")
	b.WriteString("  em `01-items.md`: uma combinação com dois itens do mesmo grupo é impossível\n")
	b.WriteString("  de executar, por melhor que os números pareçam.\n\n")
	...
	fmt.Fprintf(&b, "- **%d grupos de exclusividade restringem a loja**, e as combinações de\n",
		len(ds.GruposDeItem))
	b.WriteString("  `05-computed.md` já os respeitam. Eles vêm do dump do jogo: o catálogo do\n")
	b.WriteString("  cliente não publica essa regra em campo nenhum, e sem ela o cálculo devolvia\n")
	b.WriteString("  o ótimo exato de um conjunto que a loja não vende.\n")
